...
int main(int argc, char* argv[]) {
    ...
    //htons or htonl might be needed             //  (set server location)  
    int len = sizeof(server_address);
    
    socket_id = socket(AF_INET, SOCK_STREAM, 0);  //2. get phone (get socket)

    int result = 
          connect(socket_id,(struct sockaddr*)&server_address, len);   
    ...
    while( (read_count = read(socket_id, buffer, BUFSIZ)) > 0) //get response
            write(1,buffer, read_count);                       //print response
    close(socket_id);                    //5. hangup phone
    return 0;   
}
